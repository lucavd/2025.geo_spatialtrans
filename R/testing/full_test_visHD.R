...
cfg <- list(
  ...
  n_cells = 90000,         # Target celle per dataset medio
  ...
  pixel_size_um = 2.5,      # 10μm per spot Visium, 2.5 per Visium HD
  grid_mode = TRUE,
  grid_resolution = 2.5,    # Più denso per full test
  grid_spacing = 0,
  use_fixed_grid = TRUE,
  fixed_grid_width_mm = 6.5,   # Area più grande
...
diff_cfg <- list(
  ...
  dropout_params = list(
    dropout_range = c(0.25, 0.45),   # Range dropout realistico per spatial
