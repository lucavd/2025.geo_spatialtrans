...
# Script per la simulazione di dati di trascrittomica spaziale
# Genera dati simulati con correlazione spaziale e di espressione genica
...
generate_expression_data <- function(n_cells, n_genes, spatial_cor, correlation_level) {
  ...
  # Trasformazione in conteggi usando distribuzione Poisson
  expression_data <- exp(expression_data)
  expression_data <- matrix(rpois(length(expression_data), expression_data),
                            nrow = n_cells)
