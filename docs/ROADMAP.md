...
Le densità e i raggi qui sotto sono **ipotesi di partenza** da confermare in `BIO_REFERENCES.md` (binario R, sessione R1-R2), non valori accertati.
...
| R1 | — | prossima | — | inventario e download dataset Visium HD pubblici; nessuna dipendenza |

**Prossima sessione: R1** (inventario dataset reali). Da S1.2 in avanti nessuno step del binario S parte se il riferimento R corrispondente non è pronto; S1.1 (`extract_regions()`) non ha dipendenze dal binario R e può alternarsi con R1–R2.
